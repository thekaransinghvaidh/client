            ...
            <div className="container mx-auto max-w-6xl">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <nav className="bg-white rounded-[2rem] p-4 shadow-xl shadow-gray-200/40 border border-gray-100 lg:sticky lg:top-32">
                            <div className="space-y-2">
                                {navItems.map((item) => (
                                    <button
                                        key={item.id}
                                        onClick={() => setActiveTab(item.id)}
                                        className={`w-full flex items-center justify-between px-5 py-4 rounded-2xl font-bold text-sm transition-all active:scale-95 ${activeTab === item.id
                    ...
                    <div className="lg:col-span-3 space-y-8">
                                    ...
                                    <div className="relative p-8 md:p-12 rounded-[2.5rem] bg-gradient-to-br from-[#FAFAFA] to-white border-2 border-dashed border-ayur-beige/30 flex flex-col items-center text-center group">
                                        {ordersLoading ? (
                                            ...
                                            <div className="w-full space-y-4 text-left">
                                                ...
                                                {orders.length > 3 && (
                                                    <button onClick={() => setActiveTab('orders')} className="w-full text-center text-ayur-green font-bold text-sm py-2 hover:underline">
                                                        View All Orders
                                                    </button>
                                                )}
                                            </div>
                        ...
                        {activeTab === 'orders' && (
                            <section className="bg-white p-8 md:p-10 rounded-[3rem] shadow-xl shadow-gray-200/40 border border-gray-100 min-h-[400px]">
                                <h3 className="text-2xl font-serif text-ayur-green font-bold mb-8">My Orders</h3>
                                {ordersLoading ? (
                                    <div className="flex flex-col items-center justify-center py-20">
                                        <Loader2 size={40} className="animate-spin text-ayur-green mb-4" />
                                        <p className="text-gray-400">Loading your order history...</p>
                                    </div>
                                ) : orders.length > 0 ? (
                                    <div className="space-y-4">
                                        {orders.map((order) => (
                                            <div key={order._id} className="bg-gray-50/50 p-6 rounded-3xl border border-gray-100 hover:border-ayur-green/20 transition-all">
                                                ...
                                                <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between">
                                                    <div className="text-sm text-gray-500">
                                                        Placed on <span className="font-bold text-gray-700">{new Date(order.createdAt).toLocaleDateString()}</span>
                                                    </div>
                                                    <div className="text-right">
                                                        <div className="text-xs text-gray-400 font-bold uppercase">Total Amount</div>
                                                        <div className="text-lg font-bold text-ayur-green">₹{order.totalPrice}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                        ...
                        {activeTab === 'profile' && (
                            <section className="bg-white p-8 md:p-10 rounded-[3rem] shadow-xl shadow-gray-200/40 border border-gray-100 min-h-[400px]">
                                <h3 className="text-2xl font-serif text-ayur-green font-bold mb-8">Health Profile</h3>
                                <div className="max-w-2xl bg-gray-50/50 p-8 rounded-[2.5rem] border border-gray-100">
                                    <div className="space-y-6">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            ...
                                            <div>
                                                <label className="text-[10px] text-gray-400 font-bold uppercase tracking-widest block mb-1">Email Address</label>
                                                <div className="text-lg font-bold text-gray-900">{userInfo.email}</div>
                                            </div>
                                            <div>
                                                <label className="text-[10px] text-gray-400 font-bold uppercase tracking-widest block mb-1">Account Status</label>
                                                <div className="flex items-center gap-2 text-ayur-green font-bold">
                                                    <ShieldCheck size={18} /> Verified Patient
                                                </div>
                                            </div>
                                            <div>
                                                <label className="text-[10px] text-gray-400 font-bold uppercase tracking-widest block mb-1">Member Since</label>
                                                <div className="text-lg font-bold text-gray-900">January 2026</div>
                                            </div>
                                        </div>
                                        <div className="pt-6 border-t border-gray-200">
                                            <p className="text-sm text-gray-500 leading-relaxed italic">
                                                * Your profile helps us personalize your Ayurvedic journey. We ensure your medical data stays private and secure.
                                            </p>
                                        </div>
                                    </div>
                                </div>
